...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def march(str):
        for word in split(lower(remove_punctuation(str))):
            if word in subject:
                return True
        return False
    return march
    ...
    if len(typed_words) == 0 and len(source_words) == 0:
        ...
        for index in range(min(len(typed_words),len(source_words))):
            if typed_words[index] == source_words[index]:
                march_num += 1
        return march_num / len(typed_words) * 100
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        def diff_helper(word):
            return diff_function(typed_word, word, limit)
        sim = min(word_list, key = diff_helper)
        if diff_function(typed_word, sim, limit) <= limit:
            return sim
        else:
            return typed_word
    # END PROBLEM 5
...
def minimum_mewtations(typed, source, limit):
    ...
    #print("Debug : typed = " + typed + ' ' + source, limit)
...
def report_progress(typed, source, user_id, upload):
    ...
    "*** YOUR CODE HERE ***"
    flag, index = 0, 0
    while not flag and index <= len(typed) - 1:
        if typed[index] != source[index]:
            flag = 1
        else:
            index += 1
    progress = index / len(source)
    diction = {'id':user_id, 'progress': progress}
    upload(diction)
    print(progress)
    # END PROBLEM 8
...
def time_per_word(words, timestamps_per_player):
    ...
    "*** YOUR CODE HERE ***"
    dur = [
        [timestamps_per_player[player][i+1] - timestamps_per_player[player][i] for i in range(len(words))]
...
def time_per_word_match(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    word , dur = time_per_word(words, timestamps_per_player)
    return match(word, dur)
    # END PROBLEM 10
...
def fastest_words(match_object):
    ...
    # BEGIN PROBLEM 11
    "*** YOUR CODE HERE ***"
    word = get_all_words(match_object)
    times = get_all_times(match_object)
    fastword = [[] for i in player_indices]
    for i in word_indices:
        fast_player = min(player_indices, key = lambda p: times[p][i])
        fastword[fast_player].append(word[i])
    return fastword
